...
CControllerFactory::CControllerFactory(std::shared_ptr<IObjectFactory> m_copy_factory) : m_factory(std::move(
    m_copy_factory)) {
  if (CGlobalGame::Instance()->isWith_graphics())
    m_texture =
        std::make_shared<SDL2pp::Texture>(CGlobalGame::Instance()->CurRenderer(),
                                          this->m_factory->getM_properties()["icon"].get<std::string>());
  else
    m_texture = nullptr;
}
